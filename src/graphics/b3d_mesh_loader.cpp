...
SP::SPMesh* B3DMeshLoader::toSPM(scene::CSkinnedMesh* mesh)
{
    ...
    for (unsigned b = 0; b < all_buf.size(); b++)
    {
        ...
        if (all_buf[b]->Material.getTexture(0))
        {
            tex_name_1 = reinterpret_cast<char*>
                (all_buf[b]->Material.getTexture(0));
        }
        if (all_buf[b]->Material.getTexture(1))
        {
            tex_name_2 = reinterpret_cast<char*>
                (all_buf[b]->Material.getTexture(1));
        }
...
void B3DMeshLoader::loadTextures(SB3dMaterial& material)
{
    ...
    for (u32 i=0; i<video::MATERIAL_MAX_TEXTURES; ++i)
    {
        ...
        if (B3dTexture && B3dTexture->TextureName.size() && !material.Material.getTexture(i))
        {
            ...
            if (CVS->isGLSL())
            {
                const unsigned ss = full_path.size() + 1;
                char* string_ptr = new char[ss];
                memcpy(string_ptr, full_path.c_str(), ss);
                m_string_ptrs.push_back(string_ptr);
                tex = reinterpret_cast<video::ITexture*>(string_ptr);
            }
            else
            {
